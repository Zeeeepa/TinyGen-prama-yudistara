...
echo -e "${YELLOW}Please enter your DeepInfra API key (press Enter to use default key):${NC}"
read -r DEEPINFRA_API_KEY

# Use default key if none provided
if [ -z "$DEEPINFRA_API_KEY" ]; then
    DEEPINFRA_API_KEY="Fe3V9w1bWf50qX6IeBtsvqLqxIDhyzyE"
    echo -e "${YELLOW}Using default DeepInfra API key.${NC}"
fi
